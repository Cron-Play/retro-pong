...
export default function ProfileScreen() {
  ...
  const fetchProfile = async () => {
    ...
    setLoading(true);
    try {
      // TODO: Backend Integration - GET /api/profile → { id, email, name, gameTag, avatar, wins, losses, totalGames }
      // Placeholder data
      ...
      setEditGameTag(mockProfile.gameTag);
    } catch (error) {
  ...
  const handleSaveProfile = async () => {
    console.log("Saving profile:", { name: editName, gameTag: editGameTag });
    setSaving(true);
    try {
      // TODO: Backend Integration - PUT /api/profile with { name, gameTag } → updated profile
      // Update local state
      if (profile) {
        setProfile({ ...profile, name: editName, gameTag: editGameTag });
      }
      setEditMode(false);
    } catch (error) {
